...
from.formula import getPerString
    ...
    def post(self, request, format=None):
        ...
        for item in query_result:
            singlePrediction = {
                ...
                "per_prob" :"chances: "+ getPerString(int(frontendData["rank"]), item.cutoff, item.seats)+"%"
            }
...
class Rank(APIView):
    ...
    def post(self, request, format=None):
        ...
        unranks = read_rank_csv("./rank/scripts/datas/filtered_final_2.csv")
        missing_ranks, unadmitted_students = find_missing_ranks(unranks, int(min_rank), int(max_rank))
        response_data = {
            "data": queryset,
            "unadmitted_students": unadmitted_students,
...
def read_rank_csv(file_path):
    with open(file_path, newline="") as csvfile:
        reader = csv.reader(csvfile)
        next(reader)
        unranks = [int(row[10]) for row in reader if row[10].isdigit()]  # Assuming rank is in the 11th column (index 10)
    return unranks

def find_missing_ranks(unranks, min_rank, max_rank):
    all_possible_ranks = set(range(min_rank, max_rank + 1))
    missing_ranks = sorted(list(all_possible_ranks - set(unranks)))
    return missing_ranks, len(missing_ranks)



# unranks=read_rank_csv("./rank/scripts/datas/filtered_final_2.csv")
# missing_ranks=find_missing_ranks(unranks)
# print("Missing ranks:")
